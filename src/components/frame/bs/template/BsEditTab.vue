  ...
  async mounted() {
    if (this.tab.table && this.tab.table.cols.length) {
      this.tab.table.cols.forEach(v => {
        ...
        if ((this.tab.type === 'view' || this.tab.type === 'index') && v.attrs) {
          v.attrs.disabled = true
        }
      })
      ...
      for (const v of this.tab.table.cols) {
        // 非树非远程查询且第一次查询
        if (v.attrs && v.attrs.data && !v.attrs.data.includes('TREE')) {
          if (v.attrs.cacheable === undefined || v.attrs.cacheable === null || v.attrs.cacheable) {
            let cacheKey = JSON.stringify({ data: v.attrs.data, params: v.attrs.params, dataAuth: v.attrs.dataAuth })

            let dataList = await this.$store.dispatch('GetBaseSelectList', cacheKey)
            if (!dataList) {
    ...
    if (this.tab.type !== 'add' && this.tab.type !== 'index' && !this.$route.params.billHis) this.getList()
  },
  methods: {
    // 初始化数据
    getList() {
