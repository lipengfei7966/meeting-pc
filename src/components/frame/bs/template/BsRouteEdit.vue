...
const registerComponentTab = data => {
  data.forEach(item => {
    if (item.component && item.element !== 'base-dialog-select' && item.element !== 'base-select') {
      if (item.uuid === undefined) {
        let compKey = item.component.toString()
        if (/^function\(\){return new Promise\(function\(\w\){\w\(\)}\)\.then\(\w\.bind\(null,"([^"]+)"\)\)}$/.test(compKey)) {
          // 提取组件路径 prod模式 es5
          compKey = compKey.replace(/^function\(\){return new Promise\(function\(\w\){\w\(\)}\)\.then\(\w\.bind\(null,"([^"]+)"\)\)}$/, '$1')
          compKey = CryptoJS.enc.Utf8.parse(compKey)
          compKey = 'bs-temp-p-' + CryptoJS.enc.Hex.stringify(compKey) + 't'
        } else if (/^\(\)=>new Promise\(function\(\w\){\w\(\)}\)\.then\(\w\.bind\(null,"([^"]+)"\)\)$/.test(compKey)) {
          // 提取组件路径 prod模式 es6
          compKey = compKey.replace(/^\(\)=>new Promise\(function\(\w\){\w\(\)}\)\.then\(\w\.bind\(null,"([^"]+)"\)\)$/, '$1')
          compKey = CryptoJS.enc.Utf8.parse(compKey)
          compKey = 'bs-temp-p-' + CryptoJS.enc.Hex.stringify(compKey) + 't'
        } else if (/^[^"']+__webpack_require__\(["']([^"']+)["']\)\)$/.test(compKey)) {
          // 提取组件路径 dev模式
          compKey = compKey.replace(/^[^"']+__webpack_require__\(["']([^"']+)["']\)\)$/, '$1')
          compKey = CryptoJS.enc.Utf8.parse(compKey)
          compKey = 'bs-temp-p-' + CryptoJS.enc.Hex.stringify(compKey) + 't'
        } else {
          compKey = item.name + window.location.hash
          compKey = CryptoJS.enc.Utf8.parse(compKey)
          compKey = 'bs-temp-u-' + CryptoJS.enc.Hex.stringify(compKey) + 't'
        }
        item.uuid = compKey
      }
      if (!Vue.component(item.uuid)) {
        Vue.component(item.uuid, item.component)
      }
    }
  })
}
  ...
  beforeMount() {
    ...
    if (Array.isArray(this.edit.tabs)) {
      if (attachmentFlag) {
        this.edit.tabs.push({
          ...
          component: () => import('@/components/frame/bs/Attachment/billTab')
        })
      }
      registerComponentTab(this.edit.tabs)
    } else {
      if (attachmentFlag) {
        this.edit.tabs = []
        this.edit.tabs.push({
    ...
    if (Array.isArray(this.edit.formData)) {
      this.edit.formData.forEach(v => {
        ...
        if (this.edit.type === 'view' && v.attrs) {
  ...
  methods: {
    ...
    getEditData() {
      ...
      request({
        ...
        .then(response => {
          ...
          // 初始化数据回调
          if (this.edit.initType && this.edit.initType === 'selfInit') {
    ...
    commonValidate(buttonInfo) {
      if (buttonInfo.validate) {
        const result = buttonInfo.validate(this.editForm)
    ...
    quickConfirm(buttonInfo) {
      ...
      if (buttonInfo.validateApi) {
        ...
        this.confirmQuickConfirm(buttonInfo)
      }
    },
    // 确认操作
    confirmQuickConfirm(buttonInfo) {
      let setData
      if (buttonInfo.getParam) {
        setData = buttonInfo.getParam(this.editForm)
      } else {
        this.$notify(
          notifyError({
            msg: '缺少操作参数信息'
      ...
      confirmTip(this, 'confirmSet')
        .then(() => {
          this.loading = true
          request({
            url: buttonInfo.url || '/api/bill/bizBill/quickConfirm',
            method: 'POST',
            data: {
              data: setData,
              funcModule: this.$t('route.' + this.$route.meta.title),
              funcOperation: this.$t('biz.btn.quickConfirm')
            }
          })
            .then(response => {
              this.$notify(notifySuccess({ msg: '确认成功' }))
              this.loading = false
              this.getEditData()
            })
            .catch(() => {
    ...
    update(buttonInfo) {
      // 后端验证
      if (buttonInfo.validateApi) {
        const param = buttonInfo.validateApi(this.edit.param)
        if (param && param.url) {
          request({
            url: param.url,
            method: 'POST',
    ...
    confirmUpdate(buttonInfo) {
      this.$router.push(buttonInfo.routeInfo(this.edit.param))
    },
    // 保存通用事件
    ...
    attachment(buttonInfo) {
      ...
      Vue.component('view-form-edit', () => import('@/components/frame/bs/Attachment/index.vue'))
      this.dialogLv2Visible = true
      $(document).ready(() => {
        $(this.$refs.viewFormEdit.$el)
          .find('.mask')
    ...
    print(buttonInfo) {
      this.edit.opType = this.edit.type === 'add' ? 'update' : this.edit.type
      Vue.component('view-form-edit', () => import('@/components/frame/bs/template/dialog/BsDialogPrint.vue'))
      if (buttonInfo.getParam) {
