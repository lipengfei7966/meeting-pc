  ...
  <div class="bs-new-container app-container tree-form-table">
    ...
    <div class='left-content'>
      ...
      <div class='tree-wrapper'>
        <!-- :data="treeData" -->
        <u-table use-virtual fixed-columns-roll :default-expand-all="true" :row-id="treeTableData.form.table.rowKey" stripe border class='table-content' ref="tree" :row-height="24" highlight-current-row element-loading-spinner="el-icon-loading" :element-loading-text="$t('route.load')" :height='tableHeight - 96' @current-change="handleCheckChange" @row-click="handleNodeClick">
          <u-table-column align='center' v-if="treeTableData.form.table.showIndex" type="index" fixed="left" width="50" :label='$t("table.id")'></u-table-column>
    ...
    <main class='right-content'>
      ...
      <u-table use-virtual fixed-columns-roll :default-expand-all="true" :row-id="treeTableData.mainData.table.rowKey" stripe border class='table-content' ref="singleTable" highlight-current-row v-loading="loading" element-loading-spinner="el-icon-loading" :element-loading-text="$t('route.load')" :height='tableHeight - 96' :row-height="24" @current-change="handleSelectRow">
    ...
    getTree() {
      request({
        ...
        .then(response => {
          ...
          this.$nextTick(() => {
            this.$refs.tree.reloadData(this.treeData)
          })
        })
        .catch(() => {})
    },
