  ...
  methods: {
      ...
      switch (this.type) {
        ...
        default:
          ...
          if (this.input && this.input * 1) {
            ...
            const decimal = new Decimal(this.input)
            this.input = decimal.toFixed(this.precision, Decimal.ROUND_HALF_UP)
          }
    ...
    thousandsHandle(max, min, precision) {
      ...
      if ((this.input && this.input * 1) || (this.input === '0' && precision)) {
        ...
        const decimal = new Decimal(this.input)
        this.input = decimal.toFixed(precision, Decimal.ROUND_HALF_UP)
      }
      // 判断是否在可输入范围内，不在范围则清空，边界值允许输入
