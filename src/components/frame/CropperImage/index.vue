    ...
    <div class="cropper-box">
      ...
      <div class="footer-btn">
        ...
        <div class="upload-btn">
          <el-button size="mini" type="success" @click="uploadImg">上传封面 <i class="el-icon-upload"></i></el-button>
        </div>
      </div>
    </div>
    <!--预览效果图-->
...
export default {
  ...
  props:['fileName', 'filePath', 'limitWidth', 'limitHeight'],
  data() {
    return {
      name:this.fileName,
      previews: {},
      option:{
        img: this.filePath,             //裁剪图片的地址
        outputSize: 1,       //裁剪生成图片的质量(可选0.1 - 1)
        outputType: 'jpeg',  //裁剪生成图片的格式（jpeg || png || webp）
        info: true,          //图片大小信息
        canScale: true,      //图片是否允许滚轮缩放
        autoCrop: true,      //是否默认生成截图框
        autoCropWidth: this.limitWidth,  //默认生成截图框宽度
        autoCropHeight: this.limitHeight, //默认生成截图框高度
