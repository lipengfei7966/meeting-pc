<template>
  <bs-dialog :dialog='dialog' :initFun='initFun' ref='bsDialog' @closeDialog='handleCloseDialog' @handleNode='handleNode'></bs-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    handleNode(node) {
      if (node) {
        if (node.deptCode) {
          this.$refs.bsDialog.formData.orgCode = node.orgCode
          this.dialog.formData.forEach(item => {
            if (item.prop === 'orgCode') {
              item.attrs.disabled = true
            }
    ...
    changeAllOrg(item) {
      this.$refs.bsDialog.formData.parentCode = ''
