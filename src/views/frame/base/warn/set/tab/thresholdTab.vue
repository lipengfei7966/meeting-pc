<template>
  <bs-edit-tab v-if="showTemplate" ref='bsEditTable' :tab='tabData' @initCallback='handleInitCallback' @closeHandler='handleClose'></bs-edit-tab>
</template>
        ...
        .then(() => {
          request({
            ...
            data: {
              funcModule: this.$t('route.' + this.$route.meta.title),
              funcOperation: this.$t('biz.btn.search'),
              data: this.$refs.bsEditTable.currentRow.batchId
            }
          })
            .then(response => {
              this.loading = false
