    ...
    <div class='dialog-container' style="width:1183px;">
      ...
      <div class='dialog-content dialog-container__content' :style="{maxHeight: clientHeight - 80 + 'px', overflowY: 'auto'}">
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="24">
                ...
                <el-form-item label="sql注意说明2">
                  <el-input :disabled="true" value='2、SQL例子select xx from table where 1=1 #AUTH_SQL# #INNER_SQL# #TENANT_CODE#'>
                  </el-input>
                </el-form-item>
                <el-form-item label="sql注意说明3">
                  <el-input :disabled="true" value='3、多表查询时，维护的SQL字段要添加前缀'>
                  </el-input>
                </el-form-item>
              </el-col>
              <el-col :span="6">
        ...
        <header :style=" {'width': clientWidth < 1366 ? (sidebar.opened ? '1163px' : '1323px' ) : 'auto' , 'border-bottom-width' : comboxInfoVisible ? '1px' : '0' }">
          ...
          <el-table v-if="comboxInfoVisible" stripe border class='table-content tb-edit' ref="singleTable" highlight-current-row element-loading-spinner="el-icon-loading" element-loading-text="" :height='105' :data="page.pageDataJson" @current-change="handleSelectRowCombox" @selection-change="handleComboxSelectionChange">
            ...
            <el-table-column show-overflow-tooltip width="140">
              ...
              <template slot-scope='scope'>
                <input-validate v-model="scope.row.queryProp" size='mini' placeholder='内置SQL要带表前缀'></input-validate>
              </template>
            </el-table-column>
          ...
          <el-tabs type="border-card" v-model="activeName" value="form">
            <el-tab-pane label='更多查询页面-查询条件' name="form">
              ...
              <el-table stripe border class='table-content tb-edit' ref="singleTable" highlight-current-row element-loading-spinner="el-icon-loading" element-loading-text="" :height='300' :data="page.codeDataJson.form.formData" @current-change="handleSelectRowForm" @selection-change="handleFormSelectionChange">
                ...
                <el-table-column show-overflow-tooltip prop='dbType' width="120">
                  <template slot="header" slot-scope="scope">
                    数据类型
                    <span style='color:#f56c6c;' :data-header='scope'>*</span>
                  </template>
                  <template slot-scope='scope'>
                    <el-select v-model="scope.row.dbType" size='mini' clearable placeholder="">
        ...
        if (valid) {
          this.$confirm('确认是否保存', '提示', {
            ...
            params.codeDataJson.form.formData.forEach(form => {
              ...
              if (form.datadict) {
                form.attrs.datadict = form.datadict
              }
              if (form.propArry) {
