<template>
  <bs-dialog-tree ref="bsTable" :dialog='dialog' @closeDialog='handleCloseDialog' @initCallback='handleInitCallback'></bs-dialog-tree>
</template>
...
export default {
  ...
  methods: {
    ...
    handleInitCallback(data) {
      ...
      request({
        ...
        data: {
          ...
          data: { moduleCode: this.param.processDefineKey }
        }
      })
        .then(response => {
          if (response.data) {
            ...
            // 初始化数据默认选中
            this.$refs.bsTable.defaultCheckedKeys = checkedEmployee
          }
        })
        .catch(() => {})
    },
    ...
    // 保存通用事件
    save() {
      if (!this.$refs.bsTable.checkedKeys || this.$refs.bsTable.checkedKeys.length < 1) {
        this.$notify(
          notifyInfo({
      ...
      this.$confirm(this.$t('biz.msg.confirmSave'), this.$t('biz.msg.tip'), {
        ...
        this.$refs.bsTable.checkedNodes.forEach(node => {
        ...
        request({
          ...
          data: {
            data: {
              moduleCode: this.param.processDefineKey,
