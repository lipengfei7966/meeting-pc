<template>
  <bs-edit-tab :tab='tab' ref='bsTabTable'></bs-edit-tab>
</template>
  ...
  methods: {
    ...
    refresh() {
      ...
      request({
        ...
        .then(response => {
          this.$refs.bsTabTable.tableData = response.data
        })
        .catch(() => {
          this.loading = false
        })
    ...
    // 编排流程图
    editProcess() {
      const row = this.$refs.bsTabTable.currentRow
      const firstRow = this.$refs.bsTabTable.tableData[0]
    ...
    adjust() {
      ...
      const row = this.$refs.bsTabTable.currentRow
      const firstRow = this.$refs.bsTabTable.$data.tableData[0]
    ...
    delete() {
      const row = this.$refs.bsTabTable.currentRow
      const firstRow = this.$refs.bsTabTable.tableData[0]
      if (row && row === firstRow && row.enableFlg === '0' && this.$refs.bsTabTable.$data.tableData.length > 1) {
      ...
      } else if (row && row === firstRow && this.$refs.bsTabTable.$data.tableData.length <= 1) {
        this.$notify(
          notifyError({
            msg: '不能删除唯一的流程版本'
          })
