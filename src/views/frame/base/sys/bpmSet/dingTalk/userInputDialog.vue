<template>
  <bs-dialog-edit ref='bsEdit' :edit='edit' @closeDialog='handleCloseDialog'></bs-dialog-edit>
</template>
...
export default {
  ...
  methods: {
    ...
    doSubmit() {
      const validateArr = []
      const editForm = this.$refs.bsEdit.editForm
      if (Array.isArray(this.$refs.bsEdit.$refs.refForm)) {
        this.$refs.bsEdit.$refs.refForm.forEach(r => {
          r.validate(valid => {
            validateArr.push(valid)
          })
        })
      } else {
        this.$refs.bsEdit.$refs.refForm.validate(valid => {
          validateArr.push(valid)
        })
      }
      ...
      const dingCcUserList = this.$refs.bsEdit.$refs.dingCcUserList[0].$refs.tab.tableData
    ...
    callbackOfApprovalForm(callbackData, prop) {
      const formDataRef = this.$refs.bsEdit.editForm
      const billIdArray = []
      const billNmArray = []
