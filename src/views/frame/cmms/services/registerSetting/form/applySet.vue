  ...
  <div>
    <div class="applySet" :style="{ height: $parent.formSetHeight + 57 + 'px' }">
      <div class="formSet">
        ...
        <h2>{{ eventName }}</h2>
        <el-form ref="contactForm" label-position="right" :model="setForm" label-width="150px" class="contactForm" style="width: 50%; margin: 0 auto">
          ...
          <div v-show="isRegisterSetShow">
            ...
            <el-form-item v-if="applySetForm.isVerification" label="注册验证" prop="registerVerification">
              <el-checkbox-group v-model="applySetForm.registerVerification">
                <el-checkbox v-for="item in registerVerificationOptions" :key="item.dispOrder" :label="item.dispOrder" :checked="applySetForm.registerVerification.includes(item.dispOrder + '')">
                  {{ item.dictItemName }}
                </el-checkbox>
            ...
            <el-form-item v-if="applySetForm.isVerification" label="登录验证" prop="loginVerification">
              <el-checkbox-group v-model="applySetForm.loginVerification">
                <el-checkbox v-for="item in loginVerificationOptions" :key="item.dispOrder" :label="item.dispOrder" :checked="applySetForm.loginVerification.includes(item.dispOrder + '')">
                  {{ item.dictItemName }}
                </el-checkbox>
                <!-- <el-checkbox label="短信验证"></el-checkbox>
                <el-checkbox label="邮箱验证"></el-checkbox>
            ...
            <el-form-item v-if="applySetForm.isVerification && applySetForm.loginVerification.includes(4)" label="自定义验证项" prop="coustomVerification">
              <el-checkbox-group v-model="applySetForm.coustomVerification">
                <el-checkbox v-for="item in customizeOptions" :key="item.dispOrder" :label="item.dispOrder" :checked="applySetForm.coustomVerification.includes(item.dispOrder + '')">
                  {{ item.dictItemName }}
                </el-checkbox>
