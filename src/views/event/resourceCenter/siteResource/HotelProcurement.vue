<template>
  <div class="ServiceProviderProcurement" v-if="info">
    ...
    <el-card class="box-card" v-if="this.type == 1">
      ...
      <div class="context hotelSelected">
        ...
        <div class="list">
          <div class="item item-hotel" v-for="(item, index) in info.selecthotel" :key="item.id">
            ...
            <div>
              <!-- 有管理员 -->
              <div class="role role-update" v-if="
                  item.hotel_contact != null &&
                  item.hotel_contact.filter((contact) => contact.type == 1)
                    .length > 0
                ">
                <!-- one -->
                <span class="block-span">
                  ...
                  酒店默认接单人:
                  {{
                    item.hotel_contact.find(
                      (contact) => contact.type == 1 && contact.is_default == 1
                    ).name
                  }}
                  <span class="update-span" @click="
                      (update_market = true),
                        ...
                        openRadioData(item.hotel_contact, item.hotelid, index)
                    ">
                    修改>
                  </span>
                </span>
            ...
            <div>
              ...
              <div class="role role-update" v-if="
                  item.hotel_contact != null &&
                  item.hotel_contact.filter((contact) => contact.type == 2)
                    .length > 0
                ">
                <!-- GsoName -->
                <span class="block-span">
                  酒店抄送（GSO）:
                  <!-- {{ item.contactsGSO }} -->
                  <span v-for="(item, index) in item.hotel_contact.filter(
                      (contact) => contact.type == 2 && contact.is_default == 1
                    )" :key="index">
                    {{ item.name }}
                  </span>
                  <span class="update-span" @click="
              ...
              <!-- 没有GSO -->
              <div class="role role-update" v-else>
                <span class="block-span"> 酒店抄送GSO: 无 </span>
              </div>
            </div>
            <!--  -->
      ...
      <!-- 修改选择管理员、GSO联系人弹窗 -->
      <div class="market" v-show="update_market || update_order">
        <i class="el-dialog__close el-icon el-icon-close customCloseIcon" @click="
            (update_market = false),
              (update_order = false),
        ...
        <div class="market-table">
          ...
          <div v-show="update_GSO">
            <div class="header">
              ...
              <!-- <div
                v-if="update_market"
                class="header-s"
                @click="(insert_contact = true), (update_market = false)"
              >
                新增联系人
              </div> -->
            </div>
            <!-- 2222 -->
            <el-table ref="gsoTable" :data="update_market ? tableGsoData : orderData" @selection-change="handleSelectionChange" height="170" style="width: 100%" :show-header="false">
          ...
          <div v-show="!update_GSO">
            <div class="header">
              ...
              <div v-if="update_market && tableData.length == 0" class="header-s" @click="(insert_contact = true), (update_market = false)">
                新增联系人
              </div>
            </div>
            <!-- 酒店接单人列表 -->
            <el-table :data="update_market ? tableData : orderData" height="170" style="width: 100%" :show-header="false">
              <el-table-column label="" width="50">
                <template slot-scope="scope">
                  <el-radio v-if="update_market" :label="scope.row.phone" v-model="RSOconstantPhone" @click.native.prevent="
          ...
          <!--  -->
          <div class="foot">
            <el-button v-if="!update_GSO" type="primary" @click="ConfirmRSOChange">确定</el-button>
            <el-button v-if="update_GSO" type="primary" @click="ConfirmGSOChange">确定</el-button>
          </div>
        </div>
      </div>
      ...
      <div class="contact" v-if="update_contact || insert_contact">
        ...
        <div class="contact-form">
          <el-form :model="form" label-position="right" label-width="90px">
            ...
            ></el-form-item> -->
            <!--  -->
            <el-form-item label="RSO姓名:" prop="name">
...
export default {
  ...
  methods: {
    ...
    //修改选中酒店集团销售
    openRadioData(hotel_contact, hotel_id, index) {
      // debugger
      console.log(hotel_contact, hotel_id, index);
      ...
      if (this.update_GSO == true) {
        ...
        const _this = this;
        if (hotel_contact) {
          this.tableGsoData = hotel_contact.filter(
            (contact) => contact.type == 2
          );
          setTimeout(() => {
            this.tableGsoData.forEach((item) => {
              if (item.is_default == 1) {
                this.$refs.gsoTable.toggleRowSelection(item, true);
              }
            });
          }, 0);
        }
        ...
        // RSO（酒店默认接待人的列表）
        this.tableData = hotel_contact.filter((contact) => contact.type == 1);
        this.tableData.forEach((item) => {
          if (item.is_default == 1) {
            // this.$refs.gsoTable.toggleRowSelection(item, true);
            this.RSOconstantPhone = item.phone;
          }
        });
      }
      // 可以用hotel_id
    ...
    onRadioChange(e, row) {
      row.is_default = 1;
      this.RSOconstantPhone = row.phone;
      if (row.is_add == 1) {
        this.info.selecthotel[this.hotelindex].hotel_contact.push(row);
      }
      this.info.selecthotel[this.hotelindex].hotel_contact.forEach((item) => {
        if (item.type == 1) {
          debugger;
    ...
    ConfirmGSOChange() {
      this.info.selecthotel[this.hotelindex].hotel_contact.forEach((item) => {
        if (item.type == 2) {
          item.is_default = 0;
          this.selectGSOInfos.forEach((contact) => {
      ...
      this.update_market = false;
      this.shade_market_order = false;
    ...
    ConfirmRSOChange() {
      ...
      // this.tableData.forEach(item => {
      //   if(item.is_add == 1){
      //     this.info.selecthotel[this.hotelindex].hotel_contact.push(item)
      //   }
      // })
    },
    ...
    submit() {
      ...
      } else if (!telStr.test(form.phone)) {
        this.msg = "请输入正确的手机号";
        this.hint = true;
        this.shade_hint = true;
        return;
      ...
      this.tableData.push({
        email: form.email,
        id: null,
        inquiry_sheet_object_id: null,
        is_add: 1,
        is_default: 0,
        name: form.name,
        phone: form.phone,
        remarks: form.remarks,
        type: 1, // 1 酒店管理员、2 GSO
      });
      // this.$forceUpdate()
      console.log(this.info.selecthotel);
