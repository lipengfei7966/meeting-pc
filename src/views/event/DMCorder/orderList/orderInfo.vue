...
import { positiveInteger, positiveFloat, positiveFloatSix, formatDate } from '@/utils/common'
...
export default {
  ...
  data() {
    return {
      ...
      excelAddress: '' /* 'http://manger.ctgbs.com/zentao/file-download-363.html?zentaosid=e0001a6d90e967a37fa116b28170acea' */,
      totalPrices: ''
    }
  },
  computed: {
    ...
    dayMoney: () => {
      return function(list) {
        let money = 0
        list.forEach(item => {
          money += Number(item.provide_count) * this.positiveFloatSix(item.unitprice)
        })
    ...
    TravelAgencyMoney: function() {
      return this.positiveFloatSix((this.info.travelagency_payment_money * this.info.travelagency_payment_proportion) / 100)
    },
    // 直采服务费
    ...
    OutsideHotelMoney: function() {
      return this.positiveFloatSix((this.info.off_hotel_expenses * this.info.off_hotel_proportion) / 100)
    },
    // 总额
  ...
  methods: {
    ...
    downloadFile(file) {
      ...
      if (types2.includes(suffix)) {
        this.downloadUrl = 'https://view.officeapps.live.com/op/view.aspx?src=' + encodeURIComponent(url)
      } else if (types1.includes(suffix)) {
        this.downloadUrl = url
      } else {
        this.$message.info('文件格式不支持预览，下载后查看')
    ...
    settlement() {
      ...
      if (this.info.status === '0020-5' || this.info.status === '0020-6' || this.info.status === '0020-8') {
        // settlement_id = this.info.settlement_sheet_id;
        issubmit = 0
      }
