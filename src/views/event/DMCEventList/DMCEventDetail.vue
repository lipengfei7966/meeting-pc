  ...
  <div class="event_detail box">
    <div class="event_process mar_b20 pad_tb10">
      <div class="event_process_title flex_row justify_between align_center pad_lr20">
        <div class="flex_row align_center">
          <el-button type="text" @click="$router.replace('DMCEventList')">会议列表</el-button>
    ...
    <div style="margin: 10px 0">
      <div class="flex_row justify_between" style="font-size: 18px">
        <p>{{eventName}}</p>
        <!-- <el-dropdown @command="handleCommand" v-if="isShowCancelBtn"> -->
        <el-dropdown @command="handleCommand">
          <span class="el-dropdown-link" style="font-size: 16px; color: #2896eb;font-weight: 600;cursor: pointer;">
            更多<i class="el-icon-arrow-down el-icon--right"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            <!-- <el-dropdown-item command="1">高级设置</el-dropdown-item> -->
            <el-dropdown-item command="2">取消会议</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
      <el-steps :active="eventStatu" finish-status="finish" process-status='process' align-center style="margin-top: 20px;">
        <el-step :class="eventStatu == 0?'processing':''" title="创建会议" :description="eventStatu >= 1?'已完成':'进行中'"></el-step>
        <el-step :class="eventStatu == 1?'processing':''" title="确定采购需求" :description="eventStatu >= 2?'已完成':(eventStatu == 1?'进行中':'未开始')"></el-step>
        <el-step :class="eventStatu == 2?'processing':''" title="找供应商询价、议价" :description="eventStatu >= 3?'已完成':(eventStatu == 2?'进行中':'未开始')"></el-step>
        <el-step :class="eventStatu == 3?'processing':''" title="确认中标，生成订单" :description="eventStatu >= 4?'已完成':(eventStatu == 3?'进行中':'未开始')"></el-step>
        <el-step :class="eventStatu == 4?'processing':''" title="会后结算审批" :description="eventStatu >= 5?'已完成':(eventStatu == 4?'进行中':'未开始')"></el-step>
        <el-step :class="eventStatu == 5?'processing':''" title="会议完成" :description="eventStatu == 6?'已完成':(eventStatu == 5?'进行中':'未开始')"></el-step>
      </el-steps>
    </div>

    <div class="step_part">

      <stepOne :eventId="eventId" :eventName="eventName" :isDMC="true" :isShowCancelBtn="isShowCancelBtn" :eventStatu="eventStatu" ref="baseInfo"></stepOne>
      <stepTwo :eventId="eventId" :eventName="eventName" :isDMC="true" :isShowCancelBtn="isShowCancelBtn" ref="demandInfo"></stepTwo>
    ...
    <el-dialog :visible.sync="cancelEventDialog" width="30%" title="取消会议" align="center">
      <div style="width: 100%;text-align: left;padding-bottom: 8px;">取消会议，将同时取消询价单。您确认取消会议吗？</div>
...
<script>
import stepOne from '@/views/event/eventList/components/stepOne'
import stepTwo from '@/views/event/eventList/components/stepTwo'
// import stepThree from '@/views/event/eventList/components/stepThree'
// import stepFour from '@/views/event/eventList/components/stepFour'
export default {
  name: "eventDetail",
  data(){
    return{
  ...
  methods:{
    ...
    cancelEventTap() {
      ...
      this.requestApi({
        ...
        if(res){
          ...
          setTimeout(() => {
            this.$router.replace({ name: "DMCEventList" });
          }, 100);
        }
    ...
    handleCommand(command) {
      if(command == 1) {// 高级设置
        this.$router.push({name:'advancedsetting', query:{id: this.eventId,eventName:this.eventName}})
