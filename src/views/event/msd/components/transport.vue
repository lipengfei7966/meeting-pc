      ...
      <div class="demand_title">
        <table width="100%" cellpadding="10" cellspacing="0">
          <tr>
            ...
            <td style="width: 30%;border-left: 1px solid #e4e4e4" class="text_center pad_tb10">报价金额：<em class="t_orange">￥{{totalPrice.transportationQuoted?totalPrice.transportationQuoted.toFixed(2):'0.00'}}</em></td>
            <td style="width: 30%;border-left: 1px solid #e4e4e4" class="text_center">结算价：<em class="t_orange">￥{{totalPrice.transportationSettlement?totalPrice.transportationSettlement.toFixed(2):'0.00'}}</em></td>
            <td style="width: 20%;border-left: 1px solid #e4e4e4"></td>
          </tr>
        </table>
      </div>
      <div v-if="isShowRoom">
        <div v-for="(item, idx) in transport" :key='idx'>
          ...
          <el-table border width="100%" size="mini" class="transportTable" :data="item.data" :header-cell-style="getStyle" :cell-style="getStyle">
              ...
              <template slot-scope="scope">
                <div class="flex_row justify_between align_center">
                  ...
                  <span :class="['flex_1','text_center',(typeof scope.row.total_price == 'object' && isAddColorRed)?'colorRed':'']">
                    {{typeof scope.row.total_price=='object'?(scope.row.total_price?scope.row.total_price.toFixed(2):'0.00') : (scope.row.total_price?scope.row.total_price.toFixed(2):'0.00')}}
                  </span>
                </div>
              </template>
            </el-table-column>
...
export default {
  ...
  methods:{
    ...
    inputObjection(){
      // debugger
      this.$emit('transportObjection',this.transport)
    },
  }
}
</script>
...
.transportTable >>> .el-table__body-wrapper .el-icon-arrow-right:before {
  margin: 0 !important;
}
</style>
