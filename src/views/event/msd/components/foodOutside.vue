      ...
      <div class="demand_title">
        <table width="100%" cellpadding="10" cellspacing="0">
          <tr>
            ...
            <td style="width: 30%;border-left: 1px solid #e4e4e4" class="text_center pad_tb10">报价金额：<em class="t_orange">￥{{totalPrice.foodOutsideQuoted?totalPrice.foodOutsideQuoted.toFixed(2):'0.00'}}</em></td>
            <td style="width: 30%;border-left: 1px solid #e4e4e4" class="text_center">结算价：<em class="t_orange">￥{{totalPrice.foodOutsideSettlement?totalPrice.foodOutsideSettlement.toFixed(2):'0.00'}}</em></td>
            <td style="width: 20%;border-left: 1px solid #e4e4e4"></td>
          </tr>
        </table>
      </div>
      <div v-if="isShowRoom">
        <div v-for="(item, idx) in foodOutside" :key='idx'>
...
export default {
  ...
  methods:{
    ...
    inputObjection(){
      // debugger
      this.$emit('foodOutsideObjection',this.foodOutside)
    },
  }
}
</script>
