      ...
      <div class="demand_title">
        <table width="100%" cellpadding="10" cellspacing="0">
          <tr>
            ...
            <td style="width: 30%;border-left: 1px solid #e4e4e4" class="text_center pad_tb10">报价金额：<em class="t_orange">￥{{totalPrice.carQuoted?totalPrice.carQuoted.toFixed(2):'0.00'}}</em></td>
            <td style="width: 30%;border-left: 1px solid #e4e4e4" class="text_center">结算价：<em class="t_orange">￥{{totalPrice.carSettlement?totalPrice.carSettlement.toFixed(2):'0.00'}}</em></td>
            <td style="width: 20%;border-left: 1px solid #e4e4e4"></td>
          </tr>
        </table>
      </div>
      <div v-if="isShowRoom">
        <div v-for="(item, idx) in car" :key='idx'>
          ...
          <el-table border width="100%" size="mini" class="carTable" :data="item.data" :header-cell-style="getStyle" :cell-style="getStyle">
            ...
            <el-table-column min-width="30%">
              <template slot="header">
                <div class="flex_row justify_between align_center">
                  ...
                  <span class="flex_1 text_center">数量（车）</span>
              ...
              <template slot-scope="scope">
                <div class="flex_row justify_between align_center">
                  ...
                  <span :class="['flex_1','text_center',(typeof scope.row.total_price == 'object' && isAddColorRed)?'colorRed':'']">
                    {{typeof scope.row.total_price == 'object'?(scope.row.total_price.value?scope.row.total_price.value.toFixed(2):'0.00') : (scope.row.total_pric?escope.row.total_price.toFixed(2):'0.00') }}
                  </span>
                </div>
...
export default {
  ...
  methods:{
    ...
    inputObjection(){
      // debugger
      this.$emit('carObjection',this.car)
    },
  }
}
</script>
...
.carTable >>> .el-table__body-wrapper .el-icon-arrow-right:before {
  margin: 0 !important;
}
</style>
