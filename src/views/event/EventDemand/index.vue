<template>
  <div class="create_demand box">
    ...
    <div class="edit_base_info" v-if="!isCheck" v-show="isShow">
      已根据会议基本信息的会议时间自动匹配好需求可选日期，若有误请您先
      <span @click="goEdit">修改会议基本信息></span>
    </div>
    ...
    <div class="guestRoom_demand">
      ...
      <!--客房需求-->
      <room ref="room" v-if="dateArr.length" :dateArr="roomDate" :outDate="outDate"></room>
      <!--会场需求-->
      <event ref="event" v-if="dateArr.length" :dateArr="dateArr" :swingList="swingList"></event>
      <!--酒店内餐饮需求-->
      <foodDemand ref="food" v-if="dateArr.length" :dateArr="allDate" :foodList="foodList"></foodDemand>
      <!-- 酒店外餐饮需求 -->
      <foodOutDemand ref="foodOut" v-if="dateArr.length" :dateArr="allDate" :foodList="foodList"></foodOutDemand>
      <!--大交通-->
      <intercity v-if="dateArr.length" ref="intercity" :dateArr="allDate" :trafficType="trafficType"></intercity>
      <!--地面交通-->
      <car ref="car" v-if="dateArr.length" :carType="carType" :dateArr="allDate" :purposeList="purposeList"></car>
      <!--其他服务-->
      <other ref="other"></other>
    </div>
    <div class="flex_row justify_center mar_b20 sub_part" v-if="$route.query.isNext">
      <el-button type="primary" class="sub_btn" @click="subTap(1)">下一步</el-button>
      <!-- <el-button v-if="isCheck" type="cancel" class="sub_btn" @click="backPrePage">返回</el-button> -->
    </div>
    <div class="flex_row justify_center mar_b20 sub_part" v-else>
      <el-button v-if="!isCheck" type="primary" class="sub_btn" @click="subTap(0)">保存</el-button>
      <el-button v-if="isCheck" type="cancel" class="sub_btn" @click="backPrePage">返回</el-button>
    </div>
    <!--  -->
    <el-dialog title="保存成功" :visible.sync="nextDialog" align="center" top="200px">
      <el-table ref="multipleTable" :data="tableData" @row-click="checkedRow" @select="selectRow" fit :show-header="false" tooltip-effect="dark" style="width: 100%; margin-bottom: 20px" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="total_amount" label="金额" width="100">
          <template slot-scope="scope">
            <span style="color: #f91">￥{{ positiveFloat(scope.row.total_amount).toFixed(2) }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="name" width="100" label="状态" show-overflow-tooltip>
        </el-table-column>
      </el-table>
      <el-button size="small" v-if="isShowHotelBtn === 0" type="primary" @click="goNext(1)">找酒店采购</el-button>
      <el-button class="mar_lr20" size="small" type="primary" @click="goNext(2)">找会议服务商采购</el-button>
      <el-button class="mar_lr20" size="small" type="primary" @click="goNext(6)">更新询价单，邀请重新报价</el-button>
...
import foodDemand from './components/foodDemand'
import foodOutDemand from './components/foodOutDemand'
...
export default {
  ...
  mounted() {
    ...
    this.getDemandList().then((res) => {
      ...
      let b = getNextDate(this.dateArr[0], -2)
      let c = getNextDate(this.dateArr[0], -3)
    ...
    if (JSON.parse(sessionStorage.getItem('isSite'))) {
      console.log(JSON.parse(sessionStorage.getItem('isSite')))
      debugger
      this.isSite = JSON.parse(sessionStorage.getItem('isSite'))
      // 控制列表中的返回修改控制功能
      this.isShow = false
    }
  },
  methods: {
    ...
    subTap(val) {
      ...
      if (val === 1) {
        // hkz
        // debugger;
        if (data.room.length <= 0 && data.conference.length <= 0 && data.food.length <= 0) {
          // 酒店询价必须含有酒店内项目需求
          this.perfectDialog = true
          this.perfectMsg = '酒店询价必须含有酒店内项目需求'
          return
        }
      }
      this.requestApi({
