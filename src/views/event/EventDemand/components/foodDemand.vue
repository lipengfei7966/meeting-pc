    ...
    <div class="demand_item">
      <div class="demand_top flex_row justify_between">
        <div class="demand_name flex_row align_center">
          <h2 style="margin-right: 20px">酒店内餐饮需求</h2>
          <el-radio-group v-model="is_food_beverage" size="mini">
        ...
        <!-- <div class="budget flex_row align_center">
          ...
          <el-input v-model="food_beverage_budget" placeholder="非必填" type="text" size="small" @input="formatNum()"></el-input>
        </div> -->
      </div>
      <el-table border :data="foodBudgetList" v-show="is_food_beverage == 1">
        ...
        <el-table-column label="午餐" prop="is_important" width="150" align="center">
          <template slot-scope="scope">
            <!-- <el-select size="small" v-model="scope.row.lunch_address_type" class="demand_120">
              <el-option label="酒店内用餐" :value="0"></el-option>
               <el-option label="酒店外用餐" :value="1"></el-option>
            </el-select> -->
            <!--用餐类型-->
            <el-select v-if="scope.row.lunch_address_type == 0" size="small" v-model="scope.row.lunch_use_type" class="demand_120 mar_t10">
              <el-option :label="item.value" :value="item.key" v-for="item in foodList" :key="item.key"></el-option>
            </el-select>
            <el-input size="small" class="demand_120 mar_t10" v-model="scope.row.lunch_count" placeholder="人数" v-input-filter:int clearable></el-input>
          </template>
        </el-table-column>
        <el-table-column label="下午茶歇" prop="is_important" width="150" align="center">
          <template slot-scope="scope">
            <el-input size="small" v-model="scope.row.afternoon_break_count" placeholder="人数" v-input-filter:int clearable></el-input>
          </template>
        </el-table-column>
        <el-table-column label="晚餐" prop="is_important" width="150" align="center">
          <template slot-scope="scope">
            <!-- <el-select size="small" v-model="scope.row.dinner_address_type" class="demand_120">
              <el-option label="酒店内用餐" :value="0"></el-option>
               <el-option label="酒店外用餐" :value="1"></el-option>
            </el-select> -->
            <!--用餐类型-->
            <el-select v-if="scope.row.dinner_address_type == 0" size="small" v-model="scope.row.dinner_use_type" class="demand_120 mar_t10">
              <el-option :label="item.value" :value="item.key" v-for="item in foodList" :key="item.key"></el-option>
            </el-select>
            <el-input size="small" class="demand_120 mar_t10" v-model="scope.row.dinner_count" placeholder="人数" v-input-filter:int clearable></el-input>
          </template>
        </el-table-column>
      ...
      <el-button class="mar_20" type="primary" size="medium" @click="addFoodDemand">添加酒店内餐饮需求</el-button>
    </div>
  </div>
</template>
      ...
      foodBudgetList: [
        {
          ...
          lunch_count: '', // 午餐人数
          lunch_address_type: '', // 午餐用餐地点类型（0：酒店内用餐 1：酒店外用餐）
          lunch_use_type: '', // 午餐用餐类型（数据字典）
          afternoon_break_count: '', // 下午茶歇人数
          dinner_count: '', // 晚餐用餐人数
