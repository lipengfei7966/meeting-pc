...
export default {
  ...
  computed: {
    ...
    roomPrice() {
      ...
      this.info.roomlist.forEach((item, index) => {
        ...
        item.roomlists.forEach(itemc => {
          ...
          var amount = itemc.lineoroffline == 0 ? positiveFloatZero(itemc.settlementsumprice) : positiveFloatZero(itemc.settlement_unitprice) * positiveFloat(totalRoom)
          console.log(amount)
          subtotal_1[`sub_${index}`] += positiveFloatZero(itemc.quotationsumprice)
      ...
      this.roomincidental.forEach(item => {
        var sumPrice = item.lineoroffline == 0 ? positiveFloatZero(item.sumprice) : positiveFloatZero(item.price) * positiveFloat(item.count)
      ...
      price[3] = positiveFloatZero(price[3]) + positiveFloatSix(price_service) * positiveFloatSix(taxrate / 100)
      price[7] += '+' + positiveFloat(price[5]) + '*' + positiveFloatSix(taxrate) + '%'
      price[8] += '+' + positiveFloat(price[6]) + '*' + positiveFloatSix(taxrate) + '%'
      return price
    },
    ...
    roomDifference: () => {
      return function(item) {
        return (positiveFloatSix(item.unitprice) !== positiveFloatSix(item.settlement_unitprice) || positiveFloat(item.provide_count) !== positiveFloat(item.settlement_count)) && item.quoted_price_room_id != null && item.lineoroffline == 1
      }
    },
    //保存时验证客房数据
        ...
        this.info.roomlist.forEach(item => {
          if (item.roomlists && item.roomlists.length > 0) {
            item.roomlists.forEach((room, row_index) => {
              ...
              if (difference && (room.settlement_comments == null || room.settlement_comments == '')) this.adddifferenceerrormsg(1, item.date, row_index + 1)
            })
    ...
    conferencePrice() {
      ...
      this.info.conferencelist.forEach((item, index) => {
        ...
        item.conferencelists.forEach(itemc => {
          ...
          let itemcpset = itemc.lineoroffline == 0 ? positiveFloatZero(itemc.settlementsumprice) : positiveFloatZero(itemc.settlement_price) * positiveFloat(itemc.settlement_count)
      ...
      this.meetingcidental.forEach(item => {
        let itemcp = item.lineoroffline == 0 ? positiveFloatZero(item.sumprice) : positiveFloatZero(item.price) * positiveFloat(item.count)
        price[1].all += itemcp
        price[2] += itemcp
        if (item.isnotservice == 1) {
          //0不计入服务费,1计入服务费
      ...
      price[3] = positiveFloatZero(price[3]) + positiveFloatZero(price_service) * positiveFloatZero(taxrate / 100)
      price[7] = '+' + positiveFloat(price[5]) + '*' + positiveFloatSix(taxrate) + '%'
      price[8] = '+' + positiveFloat(price[6]) + '*' + positiveFloatSix(taxrate) + '%'
      return price
    },
    ...
    conferenceDifference: () => {
      return function(item) {
        return (positiveFloatSix(item.price) !== positiveFloatSix(item.settlement_price) || positiveFloat(item.count) !== positiveFloat(item.settlement_count)) && item.quoted_price_conference_id != null && item.lineoroffline == 1
      }
    },
    //保存时验证会场数据
        ...
        this.info.conferencelist.forEach(item => {
          if (item.conferencelists && item.conferencelists.length > 0) {
            item.conferencelists.forEach((conference, row_index) => {
              ...
              if (difference && (conference.settlement_comments == null || conference.settlement_comments == '')) this.adddifferenceerrormsg(2, item.date, row_index + 1)
            })
    ...
    foodPrice() {
      ...
      this.info.foodlist.forEach((item, index) => {
        ...
        item.foodlists.forEach(itemc => {
          ...
          let itemcp = positiveFloatZero(itemc.quotationsumprice)
          let itemcpset = itemc.lineoroffline == 0 ? positiveFloatZero(itemc.settlementsumprice) : positiveFloatZero(itemc.settlement_unitprice) * positiveFloat(itemc.settlement_count)
    ...
    foodDifference() {
      return function(item) {
        return (positiveFloatSix(item.price) !== positiveFloatSix(item.settlement_unitprice) || positiveFloat(item.personcount) !== positiveFloat(item.settlement_count)) && item.quoted_price_food_id != null && item.lineoroffline == 1
      }
    },
    //保存时验证酒店内餐饮数据
        ...
        this.info.foodlist.forEach(item => {
          if (item.foodlists && item.foodlists.length > 0) {
            item.foodlists.forEach((food, row_index) => {
              ...
              if (difference && (food.settlement_comments == null || food.settlement_comments == '')) this.adddifferenceerrormsg(3, item.date, row_index + 1)
            })
        ...
        this.info.otherfoodlist.forEach(item => {
          if (item.foodlists && item.foodlists.length > 0) {
            item.foodlists.forEach((food, row_index) => {
              ...
              if (difference && (food.settlement_comments == null || food.settlement_comments == '')) this.adddifferenceerrormsg(7, item.date, row_index + 1)
            })
            let newitem = item.foodlists.filter(w => w.quoted_price_food_id == null && w.settlement_itemname == '')
            if (newitem.length > 0) {
              newitem.forEach((food, row_index) => {
    ...
    transportationDifference() {
      return function(item) {
        return (positiveFloatSix(item.unitprice) !== positiveFloatSix(item.settlement_unitprice) || positiveFloat(item.passengercount) !== positiveFloat(item.settlement_count)) && item.quoted_price_transportation_id != null && item.lineoroffline == 1
      }
    },
    //保存时验证大交通数据
        ...
        this.info.transportationlist.forEach(item => {
          if (item.tranlists && item.tranlists.length > 0) {
            item.tranlists.forEach((transportation, row_index) => {
              ...
              if (difference && (transportation.settlement_comments == null || transportation.settlement_comments == '')) this.adddifferenceerrormsg(4, item.date, row_index + 1)
            })
    ...
    carDifference() {
      return function(item) {
        return (positiveFloatSix(item.price) !== positiveFloatSix(item.settlement_unitprice) || positiveFloat(item.carcount) !== positiveFloat(item.settlement_count)) && item.quoted_price_car_id != null && item.lineoroffline == 1
      }
    },
    //保存时验证市内交通数据
        ...
        this.info.carlist.forEach(item => {
          if (item.carlists && item.carlists.length > 0) {
            item.carlists.forEach((car, row_index) => {
              ...
              if (difference && (car.settlement_comments == null || car.settlement_comments == '')) this.adddifferenceerrormsg(5, item.date, row_index + 1)
            })
    ...
    otherPrice() {
      ...
      this.info.otherlist.forEach((item, index) => {
        ...
        // debugger
        let itemcpset = item.lineoroffline == 0 ? positiveFloatZero(item.settlementsumprice) : positiveFloatSix(item.settlement_unitprice) * positiveFloat(item.settlement_count)
        ...
        if (item.isnotservice == 1) {
          ...
          price[6] += itemcpset
          // debugger
          price[7] += '+' + positiveFloat(itemcp) + '*' + positiveFloatSix(taxrate) + '%'
          price[8] += '+' + positiveFloat(itemcpset) + '*' + positiveFloatSix(taxrate) + '%'
        }
      })
      ...
      this.otherincidental.forEach(item => {
        ...
        if (item.isnotservice == 1) {
          //0不计入服务费,1计入服务费
          otherPrice += positiveFloatZero(itemcp) * positiveFloatZero(item.settlementservice_taxrate / 100)

          price[6] += itemcp
          price[8] += '+' + positiveFloat(itemcp) + '*' + positiveFloatSix(item.settlementservice_taxrate) + '%'
        }
      })
      price[3] = positiveFloatZero(service_price) + positiveFloatZero(otherPrice)
    ...
    otherDifference() {
      return function(item) {
        return (positiveFloatSix(item.price) !== positiveFloatSix(item.settlement_unitprice) || positiveFloat(item.offerservicedaycount) * positiveFloat(item.offerparticipatecount) !== positiveFloat(item.settlement_count)) && item.lineoroffline == 1
      }
    },
    //保存时验证其他服务数据
    ...
    servicePrice() {
      ...
      if (this.info.event_info_configure.service_fee_calculation_method == 0) {
        ...
        for (var i = 0; i < this.info.event_info_grade_taxrates.length; i++) {
          if (i == this.info.event_info_grade_taxrates.length - 1 || (positiveFloatZero(oldTotalPrice) >= this.info.event_info_grade_taxrates[i].grade_money && positiveFloatZero(oldTotalPrice) < this.info.event_info_grade_taxrates[i + 1].grade_money)) {
            price[0] = (((positiveFloatZero(oldTotalPrice) * this.info.event_info_grade_taxrates[i].grade_taxrate) / 100) * this.info.fzc_service) / 100 // 服务费金额 = 计入服务费总价 * 对应档次 服务费比例
            price[2] = positiveFloatZero(oldTotalPrice) + '*' + this.info.event_info_grade_taxrates[i].grade_taxrate + '%'

            if (this.info.event_info_configure.service_fee_ratio == 1 && this.fzc_service != null) {
        ...
        // 结算服务费
        for (var i = 0; i < this.info.event_info_grade_taxrates.length; i++) {
          if (i == this.info.event_info_grade_taxrates.length - 1 || (positiveFloatZero(newTotalPrice) >= this.info.event_info_grade_taxrates[i].grade_money && positiveFloatZero(newTotalPrice) < this.info.event_info_grade_taxrates[i + 1].grade_money)) {
            // 服务费金额 = 计入服务费总价 * 对应档次 服务费比例 * 服务费折扣
            price[1] = (((positiveFloatZero(newTotalPrice) * this.info.event_info_grade_taxrates[i].grade_taxrate) / 100) * this.info.sheet_fzc_service) / 100
            price[3] = positiveFloatZero(newTotalPrice) + '*' + this.info.event_info_grade_taxrates[i].grade_taxrate + '%'

            if (this.info.event_info_configure.service_fee_ratio == 1 && this.sheet_fzc_service != null) {
      ...
      if (this.info.listqtpm) {
        this.info.listqtpm.forEach((e, i) => {
          price[4] += positiveFloatZero(e.money) * positiveFloatZero(e.proportion / 100)
          price[6] += positiveFloat(e.money) + '*' + positiveFloatSix(e.proportion) + '%\n'
        })
      }
  ...
  methods: {
    ...
    initInfo() {
      this.info.roomlist.forEach((item, index) => {
        item.roomlists.forEach(itemc => {
          if (itemc.quoted_price_room_id && itemc.lineoroffline == 1 && this.info.id == null) {
      ...
      this.info.conferencelist.forEach((item, index) => {
        item.conferencelists.forEach(itemc => {
          if (itemc.quoted_price_conference_id && itemc.lineoroffline == 1 && this.info.id == null) {
            //设置默认值，结算金额，数量默认为报价金额，数量
            itemc.settlement_price = positiveFloatSix(itemc.price)
      ...
      this.info.foodlist.forEach((item, index) => {
        item.foodlists.forEach(itemc => {
          if (itemc.quoted_price_food_id && itemc.lineoroffline == 1 && this.info.id == null) {
            //设置默认值，结算金额，数量默认为报价金额，数量
            itemc.settlement_unitprice = positiveFloatSix(itemc.price)
      ...
      this.info.transportationlist.forEach((item, index) => {
        item.tranlists.forEach(itemc => {
          if (itemc.quoted_price_transportation_id && itemc.lineoroffline == 1 && this.info.id == null) {
            //设置默认值，结算金额，数量默认为报价金额，数量
            itemc.settlement_unitprice = positiveFloatSix(itemc.unitprice)
      ...
      this.info.carlist.forEach((item, index) => {
        item.carlists.forEach(itemc => {
          if (itemc.quoted_price_car_id && itemc.lineoroffline == 1 && this.info.id == null) {
            //设置默认值，结算金额，数量默认为报价金额，数量
            itemc.settlement_unitprice = positiveFloatSix(itemc.price)
      ...
      this.info.otherlist.forEach((item, index) => {
        if (item.quoted_price_other_id && item.lineoroffline == 1 && this.info.id == null) {
          //设置默认值，结算金额，数量默认为报价金额，数量
      ...
      if (this.info.id == null) {
        ...
        if (this.info.listqtpm) {
          this.info.listqtpm.forEach((e, i) => {
            inittotal += positiveFloatZero(e.money) * positiveFloatZero(e.proportion / 100)
          })
        }
      ...
      if (this.info.liststpm && this.info.id == null) {
        ...
        this.info.liststpm.forEach((e, i) => {
          initprice += positiveFloatZero(e.money) * positiveFloatZero(e.proportion / 100)
        })
