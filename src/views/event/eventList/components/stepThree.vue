  ...
  methods: {
    ...
    excelDownLoad() {
      ...
      this.excelExport(
        ...
        { eventid: this.eventId, type: 2, param: this.eventId },
        name
      );
    },
    //跳转填写线下报价项结算
    ...
    budgetSubmit() {
      ...
      isSuspend = this.budgetInfo.costlist.some((item, index) => {
        ...
        for (var i = index + 1; i < this.budgetInfo.costlist.length; i++) {
          if (
            item.costItem == this.budgetInfo.costlist[i].costItem &&
            item.supplyId == this.budgetInfo.costlist[i].supplyId
          ) {
      ...
      for (var i = 0; i < this.budgetInfo.costlist.length; i++) {
        if (
          this.budgetInfo.costlist[i].costItem == "" ||
          this.budgetInfo.costlist[i].costItem == null
        ) {
        ...
        if (
          this.budgetInfo.costlist[i].supplyId == "" ||
          this.budgetInfo.costlist[i].supplyId == null
        ) {
        ...
        if (
          this.budgetInfo.costlist[i].cost == "" ||
          this.budgetInfo.costlist[i].cost == null
        ) {
    ...
    budget() {
      this.requestApi({
        ...
        if (this.settlementList[0] && this.settlementList[0].settlement_count)
          this.num = this.settlementList[0].settlement_count;
        this.orderIdList = [];
      });
    },
    //结算单填报(明细)
    budgetDetails() {
      this.requestApi({
        ...
        this.budgetInfo.balancelist[0].acceptMoney = Number(
