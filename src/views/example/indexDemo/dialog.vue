...
export default {
  ...
  mounted() {
    setTimeout(() => {
      ...
      // this.$refs.bsEdit.$refs.bsDialogEdit.editForm.product.startDate = '20200911'
    }, 3000)
  },
  methods: {
    // 按钮做相应验证
    validateEvent() {
      if (!this.$refs.bsEdit.$refs.bsDialogEdit.editForm.product.usingFlag) {
        this.$notify(
          notifyInfo({
            msg: this.$t('请选择状态')
          })
    ...
    // 设置打开弹窗默认值
    doSet(editForm) {
      this.$refs.bsEdit.$refs.bsDialogEdit.$refs.viewFormEdit.$refs.bsDialogTable.listQuery.usingFlag = editForm.product.usingFlag
    },
    dialogEvent() {
      Vue.component('view-form-edit', () => import('@/views/example/indexDemo/productSelect.vue'))
      this.$refs.bsEdit.$refs.bsDialogEdit.dialogLv2Visible = true
      // 防止被遮盖
      $(document).ready(() => {
        $(this.$refs.bsEdit.$refs.bsDialogEdit.$refs.viewFormEdit.$el)
