...
export default {
  ...
  methods: {
    ...
    // 按钮做相应验证
    validateEvent() {
      if (!this.$refs.bsEdit.$refs.bsRouteEdit.editForm.product.usingFlag) {
        this.$notify(
          notifyInfo({
            msg: this.$t('请选择状态')
          })
    ...
    // 设置打开弹窗默认值
    doSet(editForm) {
      this.$loopDOM(this, 'bsDialogTable').listQuery.data.usingFlag = editForm.product.usingFlag
    },
    dialogEvent() {
      Vue.component('view-form-edit', () => import('@/views/example/indexDemo/productSelect.vue'))
      this.$refs.bsEdit.$refs.bsRouteEdit.dialogLv2Visible = true
    },
